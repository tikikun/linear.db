...
export function registerLabelTools(registerHandler: (name: string, handler: (args: any) => Promise<any>) => void) {
  registerHandler("list_issue_labels", async (args) => {
    let sql = "SELECT * FROM labels WHERE 1=1";
    const params: any[] = [];
    if (args.team) { const teamId = await getTeamId(args.team); if (teamId) { sql += " AND (team_id = ? OR team_id IS NULL)"; params.push(teamId); } }
    if (args.name) { sql += " AND name LIKE ?"; params.push(`%${args.name}%`); }
    sql += " ORDER BY name LIMIT ?"; params.push(args.limit || 50);
  ...
  registerHandler("create_issue_label", async (args) => {
    ...
    await run("INSERT INTO labels (id, name, color, description, team_id, created_at) VALUES (?, ?, ?, ?, ?, datetime('now'))",
      [id, args.name, args.color || "#bec2c8", args.description || null, teamId]);
  ...
  registerHandler("update_issue_label", async (args) => {
    ...
    if (args.color !== undefined) { updates.push("color = ?"); params.push(args.color); }
    if (updates.length > 0) { params.push(args.id); await run(`UPDATE labels SET ${updates.join(", ")} WHERE id = ?`, params); }

...
import { success, error, getTeamId, getUserId } from "./base.js";
...
export function registerProjectTools(registerHandler) {
    registerHandler("list_projects", async (args) => {
        ...
        if (!args.includeArchived) {
            sql += ` AND (p.status != 'archived' OR p.status IS NULL)`;
        }
    ...
    registerHandler("get_project", async (args) => {
        const project = await getOne(`SELECT p.*, t.name as team_name, u.name as lead_name FROM projects p
        ...
        if (args.includeMilestones) {
            project.milestones = await query(`SELECT * FROM milestones WHERE project_id = ? ORDER BY target_date ASC`, [project.id]);
        }
    ...
    registerHandler("create_project", async (args) => {
        ...
        await run(`INSERT INTO projects (id, name, description, icon, color, status, lead_id, team_id, priority, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, args.name, args.description || null, args.icon || null, args.color || null, args.state || 'started', leadId, teamId, args.priority || 0, new Date().toISOString(), new Date().toISOString()]);
        return success({ id, ...args });
    });
    registerHandler("update_project", async (args) => {
        const project = await getOne(`SELECT id FROM projects WHERE id = ? OR name = ?`, [args.id, args.id]);
        if (!project)
            return error("Project not found");
        const updates = [];
        const params = [];
    ...
    registerHandler("list_milestones", async (args) => {
        const project = await getOne(`SELECT id FROM projects WHERE id = ? OR name = ?`, [args.project, args.project]);
        if (!project)
            return error("Project not found");
    ...
    registerHandler("create_milestone", async (args) => {
        ...
        await run(`INSERT INTO milestones (id, name, description, target_date, project_id, created_at) VALUES (?, ?, ?, ?, ?, ?)`, [id, args.name, args.description || null, args.target_date || null, project.id, new Date().toISOString()]);
        return success({ id, ...args });
    });
}
